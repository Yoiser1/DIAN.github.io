...
(function(H) {
    ...
    function N() {
        ...
        this.computePlotData = function() {
            ...
            var am, an, ai;
            for (an = 0,
            ai = this.series.length; an < ai; an++) {
                am = this.series[an];
                this._plotData.push([]);
        ...
        this.draw = function() {
            if (this.drawIfHidden || this.target.is(":visible")) {
                ...
                var aB, az, ay, ai;
                for (aB = 0,
                ay = H.jqplot.preDrawHooks.length; aB < ay; aB++) {
                    H.jqplot.preDrawHooks[aB].call(this)
                }
                for (aB = 0,
                ay = this.preDrawHooks.length; aB < ay; aB++) {
    ...
    aa.prototype.getTimezoneAbbr = function() {
        return this.proxy.toString().replace(/^.*\(([^)]+)\)jQuery/, "jQuery1")
    }
    ;
    aa.prototype.getTimezoneName = function() {
        var ab = /(?:\((.+)\)jQuery| ([A-Z]{3}) )/.exec(this.toString());
    ...
    aa.createDate = function(ad) {
        ...
        var ai = String(ad).replace(/^\s*(.+)\s*jQuery/g, "jQuery1");
        ai = ai.replace(/^([0-9]{1,4})-([0-9]{1,2})-([0-9]{1,4})/, "jQuery1/jQuery2/jQuery3");
        ai = ai.replace(/^(3[01]|[0-2]?\d)[-\/]([a-z]{3,})[-\/](\d{4})/i, "jQuery1 jQuery2 jQuery3");
        ...
        var ah = ai.match(/^([0-9]{1,2})[-\/]([0-9]{1,2})[-\/]([0-9]{1,2})jQuery/);
        if (ah && ah.length > 3) {
            ai = al(ai, ah)
        }
        var af = 0;
    ...
    ;
    aa.matchers = [[/(3[01]|[0-2]\d)\s*\.\s*(1[0-2]|0\d)\s*\.\s*([1-9]\d{3})/, "jQuery2/jQuery1/jQuery3"], [/([1-9]\d{3})\s*-\s*(1[0-2]|0\d)\s*-\s*(3[01]|[0-2]\d)/, "jQuery2/jQuery3/jQuery1"], function(ae) {
        var ac = ae.match(/^(?:(.+)\s+)?([012]?\d)(?:\s*\:\s*(\d\d))?(?:\s*\:\s*(\d\d(\.\d*)?))?\s*(am|pm)?\s*jQuery/i);
        if (ac) {
            if (ac[1]) {
    ...
    , function(ae) {
        var ac = ae.match(/^(?:(.+))[T|\s+]([012]\d)(?:\:(\d\d))(?:\:(\d\d))(?:\.\d+)([\+\-]\d\d\:\d\d)jQuery/i);
        if (ac) {
            if (ac[1]) {
                var ad = this.createDate(ac[1]);
                if (isNaN(ad)) {
    ...
    , function(af) {
        var ad = af.match(/^([0-3]?\d)\s*[-\/.\s]{1}\s*([a-zA-Z]{3,9})\s*[-\/.\s]{1}\s*([0-3]?\d)jQuery/);
        if (ad) {
            var ae = new Date();
    ...
    H.jqplot.sprintf = function() {
        ...
        function ae(al) {
            ...
            for (var aj = 10; aj > 0; aj--) {
                if (ak == (ak = ak.replace(/^(\d+)(\d{3})/, "jQuery1" + H.jqplot.sprintf.thousandsSeparator + "jQuery2"))) {
                    break
                }
            }
            return ak
    ...
    H.jqplot.sprintf.regex = /%%|%(\d+\jQuery)?([-+#0&\' ]*)(\*\d+\jQuery|\*|\d+)?(\.(\*\d+\jQuery|\*|\d+))?([nAscboxXuidfegpEGP])/g;
...
jQuery.extend(jQuery.jqplot.effects, {
    ...
    createWrapper: function(b) {
        ...
        var c = {
          ...
          , e = jQuery("<div></div>").addClass("ui-effects-wrapper").css({
            fontSize: "100%",
        ...
        b.wrap(e);
        if (b[0] === d || jQuery.contains(b[0], d)) {
            jQuery(d).focus()
        }
        e = b.parent();
        if (b.css("position") === "static") {
            ...
            jQuery.each(["top", "left", "bottom", "right"], function(f, g) {
                c[g] = b.css(g);
    ...
    removeWrapper: function(a) {
        ...
        if (a.parent().is(".ui-effects-wrapper")) {
            ...
            if (a[0] === b || jQuery.contains(a[0], b)) {
                jQuery(b).focus()
            }
        }
        return a
...
function _normalizeArguments(b, a, c, d) {
    ...
    if (jQuery.type(a) === "number" || jQuery.fx.speeds[a]) {
        d = c;
        c = a;
        a = {}
    }
    ...
    c = c || a.duration;
    b.duration = jQuery.fx.off ? 0 : typeof c === "number" ? c : c in jQuery.fx.speeds ? jQuery.fx.speeds[c] : jQuery.fx.speeds._default;
    b.complete = d || a.complete;
    return b
}
function standardSpeed(a) {
    if (!a || typeof a === "number" || jQuery.fx.speeds[a]) {
        return true
    }
    if (typeof a === "string" && !jQuery.jqplot.effects.effect[a]) {
        if (backCompat && jQuery.jqplot.effects[a]) {
            return false
        }
        return true
    }
...
jQuery.fn.extend({
    jqplotEffect: function(i, j, b, h) {
        var g = _normalizeArguments.apply(this, arguments)
          , d = g.mode
          , e = g.queue
          , f = jQuery.jqplot.effects.effect[g.effect]
          , a = !f && backCompat && jQuery.jqplot.effects[g.effect];
        if (jQuery.fx.off || !(f || a)) {
            if (d) {
...
jQuery.jqplot.effects.effect.blind = function(c, h) {
    var d = jQuery(this), k = ["position", "top", "bottom", "left", "right", "height", "width"], i = jQuery.jqplot.effects.setMode(d, c.mode || "hide"), m = c.direction || "up", f = rvertical.test(m), e = f ? "height" : "width", j = f ? "top" : "left", p = rpositivemotion.test(m), g = {}, n = i === "show", b, a, l;
    if (d.parent().is(".ui-effects-wrapper")) {
        jQuery.jqplot.effects.save(d.parent(), k)
    } else {
        jQuery.jqplot.effects.save(d, k)
    }
    d.show();
    l = parseInt(d.css("top"), 10);
    b = jQuery.jqplot.effects.createWrapper(d).css({
        overflow: "hidden"
    });
    a = f ? b[e]() + l : b[e]();
    g[e] = n ? String(a) : "0";
    ...
    b.animate(g, {
        ...
        complete: function() {
            ...
            jQuery.jqplot.effects.restore(d, k);
            jQuery.jqplot.effects.removeWrapper(d);
            h()
        }
    })
}
